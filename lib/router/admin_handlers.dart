...
class AdminHandlers {
  static Handler login = Handler(handlerFunc: ((context, parameters) {
    ...
    return authProvider.status == AuthStatus.unauthenticated ? const LoginView() : const DashboardView();
  }));

  static Handler register = Handler(handlerFunc: ((context, parameters) {
    final authProvider = Provider.of<AuthenticationProvider>(context!);
    return authProvider.status == AuthStatus.unauthenticated ? const RegisterView() : const DashboardView();
  }));
}
